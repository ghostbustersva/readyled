...
- **Pixel sizing**: `.readyled-sign` uses `font-size: 4px`; each pixel `<div>` is `1em × 1em`, so physical LED size = `4px × pixelSize`. Changing sign scale means adjusting `font-size` in CSS, not pixel count.
